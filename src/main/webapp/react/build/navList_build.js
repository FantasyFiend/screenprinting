...
var NavList = React.createClass({
    ...
    getInitialState: function () {
        return {
            config: {
                ...
                rootRule: "0",
                floor: 2,
                attrs: [{ name: "id", text: "id", width: "50" }, { name: "name", text: "英文名", width: "100" }, { name: "text", text: "中文名", width: "150" }, { name: "href", text: "跳转路径", width: "600" }, { name: "tagIds", text: "所包含查询条件的id", width: "150" }, { name: "parentId", text: "父级id", width: "50" }, { name: "state", text: "启用状态", width: "50" }]
            },
            list: [],
            selectedArray: []
        };
    ...
    select: function (event) {
        ...
        _selectedNav = new Array();
        for (var i = 0; i < list.length; i++) {
            var obj = list[i];
            if (_.indexOf(newArray, obj.id.toString()) > -1) {
                _selectedNav.push(obj.name);
            }
        }
...
ReactDOM.render(React.createElement(NavList, null), document.getElementById("nav_config_container"));
